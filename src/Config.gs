...
const DEFAULT_TUNABLES = {
    ...
    REPORT_BACKUP_FREQUENCY:   "daily"       // Trigger frequency for BOTH the XLSX sheet backup (weeklyBackupJob) and the PDF report job (weeklyReportJob). "daily" (default) installs an every-day trigger at the same hour as the legacy weekly slot; "weekly" preserves the historic Monday-only schedule. Re-run installWeeklyBackupTrigger + installWeeklyReportTrigger after editing this value so the existing triggers are recreated with the new cadence.
};
