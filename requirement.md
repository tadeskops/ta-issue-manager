...
| `WEEKLY_REPORT_PUBLIC_URL` | `""` | Raw URL where `TA_IAP_Report.pdf` (the anonymised public copy) lands. Recommended: `https://raw.githubusercontent.com/tadeskops/ta-issue-manager/main/backups/TA_IAP_Report.pdf`. **When empty, the server auto-derives this URL from `BACKUP_REPO` + `BACKUP_BRANCH`** (assuming the backup repo is public), so the login-page **View Report (PDF)** pill works out-of-the-box once `FEATURE_WEEKLY_REPORT_BACKUP` is on. Override only if the file is mirrored elsewhere or the backup repo is private. |
| `FULL_REPORT_PUBLIC_URL` | `""` | Raw URL where `TA_IAP_Full_Report.pdf` (the full report including names, flats, and closed/rejected rows) lands. Recommended: `https://raw.githubusercontent.com/tadeskops/ta-issue-manager/main/backups/TA_IAP_Full_Report.pdf`. **When empty, the server auto-derives this URL from `BACKUP_REPO` + `BACKUP_BRANCH`** so the dashboard **View Full Report** pill works out-of-the-box. **Privacy note:** the full file contains residents' names and flat numbers — keep the backup repo private, or override this tunable to point at an authenticated mirror, before sharing widely. |
