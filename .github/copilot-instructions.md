...
**Step 1 — Read `requirement.md` BEFORE writing code.**

- At the start of every task that could touch observable behavior
  (roles, `api_call` actions, sheet schemas, `*_COL` constants,
  config keys, feature flags, `PAGE_MAP`, setup runbook), open
  `requirement.md` and read the section(s) relevant to the request.
  Use the §1 trigger list below to identify which sections apply.
...
- Code (`.gs`, HTML partials, config), and any operator-run
  setup/migrate functions must match the spec section(s) you just
  read. If during implementation you discover the spec is ambiguous,
  incomplete, or wrong, pause and resolve the spec first (loop back
  to Step 1) — do not push code that diverges from the spec with a
  "will fix the doc later" note.

**Step 3 — Update `requirement.md` IN THE SAME COMMIT.**

- Every commit that changes observable behavior must include the
  matching `requirement.md` edit. Never split spec updates into a
  follow-up commit; never push code (or `clasp push` to Apps Script)
  that leaves the spec stale.
